...
void autonomousLoop() {
    ...
    while (true) {
        ...
        auto [throttle, steer] = planner.calculateControl(lateral_offset, yaw_error);

        // Aplicar controles
        controls.getEngineController().set_speed(throttle * 100); // Conversão para %
        controls.getEngineController().set_steering(steer * 45);  // Ângulo máximo 45°
    }
}
