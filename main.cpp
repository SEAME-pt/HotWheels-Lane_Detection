...
public:
  ...
  void cleanup() { std::cout << "[CarControls] Cleaning up" << std::endl; }
...
public:
  KerasLaneDetector(const std::string &model_path) {
    std::cout << "[KerasLaneDetector] Initialized with model: " << model_path
              << std::endl;
  }
  cv::Mat predict(const cv::Mat &img) {
    ...
    cv::line(mask, cv::Point(0, mask.rows - 1),
             cv::Point(mask.cols / 2, mask.rows / 2), cv::Scalar(255), 2);
    return mask;
  }
};
...
void print_debug_info(const std::string &text) {
  if (!DEBUG)
    return; // Ignorar se DEBUG estiver desativado
...
extractCenterCurveWithLanes(const cv::Mat &binary_mask) {
...
double calculate_curve_curvature(const std::vector<double> &x_coords,
  ...
  double sum =
      std::accumulate(curvature.begin() + start, curvature.begin() + end, 0.0);
...
std::vector<Point2D> get_waypoints_ahead(double distance, int count,
...
cv::Mat get_camera_image(CarControls &car_controls) {
  ...
  cv::line(img, cv::Point(0, img.rows - 1),
           cv::Point(img.cols / 2, img.rows / 2), cv::Scalar(255), 2);
  return img;
}

void autonomousLoop() {
  ControlsManager controls(argc, argv);
  controls.setMode(DrivingMode::Automatic);

  while (true) {
    // Obter dados de percepção
    auto [lateral_offset, yaw_error] = laneDetector.getLaneInfo();
    auto obstacles = objectDetector.getObstacles();

    // Planejamento MPC
    MPCConfig config;
    MPCPlanner planner(config);
    auto [throttle, steer] =
        planner.calculateControl(lateral_offset, yaw_error);

    // Aplicar controles
    controls.getEngineController().set_speed(throttle *
                                             100); // Conversão para %
    controls.getEngineController().set_steering(steer *
                                                45); // Ângulo máximo 45°
  }
}
...
int main() {
  try {
    ...
    while (true) {
      auto now = std::chrono::system_clock::now();
      if (std::chrono::duration<double>(now - last_control_time).count() <
          CONTROL_PERIOD)
        continue;
      ...
      print_debug_info(
          "Vehicle state: Position (" + std::to_string(current_state.x) + ", " +
          std::to_string(current_state.y) +
          "), Velocity: " + std::to_string(current_state.velocity) +
          " m/s, Yaw: " + std::to_string(current_state.yaw));
        ...
        try {
          ...
          if (!binary_mask.empty() && cv::countNonZero(binary_mask) > 0) {
            ...
            if (!center_curve.first.empty()) {
              ...
              waypoints =
                  convert_image_points_to_world(center_curve, car_controls);
              if (waypoints.size() > 5) {
                print_debug_info("Usando " + std::to_string(waypoints.size()) +
                                 " waypoints da trajetória central");
              } else {
          ...
          print_debug_info("Erro no processamento da imagem: " +
                           std::string(e.what()));
          waypoints = get_waypoints_ahead(2.0, 20, current_state);
        }
      } else {
      ...
      if (waypoints.empty()) {
        ...
        print_debug_info("Distância média dos waypoints: " +
                         std::to_string(avg_distance) + "m");
        if (avg_distance > 20.0) {
          print_debug_info(
              "Waypoints muito distantes, usando waypoints simples");
      ...
      try {
        ...
        ControlCommand control =
            mpc_planner.plan(current_state, global_waypoints, &lane_info);

        throttle_value = control.throttle;
        steer_value = control.steer;

        print_debug_info(
            "MPC output - Throttle: " + std::to_string(control.throttle) +
            ", Steer: " + std::to_string(control.steer));

        // Aplicar controle adaptativo baseado na curvatura
        bool is_curve =
            false; // Simulação: pode ser calculado conforme sua lógica

        // Forçar throttle mínimo adaptativo
        if (std::abs(throttle_value) < 0.1) {
          if (is_curve) {
            throttle_value = 0.2;
            print_debug_info("Forçando throttle mínimo para CURVA: " +
                             std::to_string(throttle_value));
          } else {
            throttle_value = 0.3;
            print_debug_info("Forçando throttle mínimo para RETA: " +
                             std::to_string(throttle_value));
          }
        }
        ...
        print_debug_info("Controle adaptativo - Throttle: " +
                         std::to_string(throttle_value) +
      ...
      if (!is_in_recovery && last_positions.size() >= 3) {
        ...
        bool is_stuck = (avg_movement < 0.05 && throttle_value > 0.2 &&
                         current_state.velocity < 0.5);

        if (is_stuck) {
          print_debug_info(
              "Car is stuck! Avg movement: " + std::to_string(avg_movement) +
        ...
        if (now < recovery_end_time) {
          double progress =
              std::chrono::duration<double>(
                  now - (recovery_end_time - std::chrono::seconds(2)))
                  .count() /
              2.0;
