...
class MPCIntegratedApp : public QObject
{
  ...
  private slots:
        ...
        try
        {
            ...
            if (step_counter % 20 == 0)
            {
                std::cout << "MPC Step " << step_counter << " | Throttle: " << std::fixed
                          ...
                          << " | Steering: " << control.steer << " | Target Speed: " << mpc_speed
    ...
    void handleKeyPress()
    {
        ...
        {
            std::cout
                << "Comando não reconhecido. Use: m(modo), r(gravar), c(limpar), s(status), q(sair)"
                << std::endl;
        }
    }
  ...
  private:
    void setupKeyboardInput()
    {
        ...
        connect(input_timer, &QTimer::timeout,
                [this]()
                {
    ...
    void toggleMPCMode()
    {
        ...
        if (mpc_active)
        {
            ...
            std::cout << "Modo MPC ATIVADO - Seguindo trajetória com " << recorded_waypoints.size()
                      << " waypoints" << std::endl;
        }
        else
        {
    ...
    void startRecording()
    {
        ...
        std::cout << "Trajetória simulada gravada com " << recorded_waypoints.size() << " pontos"
                  << std::endl;
    }
