...
// Force CUDA memory cleanup if available
				...
				try {
					...
					initial_limit =
					    std::clamp (initial_limit, 0.01, 0.3); // 1% to 30% initial limit
		...
		void getLaneDetectionFrame () {
			...
			try {
				...
				if (items[0].revents & ZMQ_POLLIN) {
					...
					if (m_laneDetectionSubscriber->getSocket ().recv (message,
		...
		void updateVisualization () {
			...
			try {
				...
				// Reorganized layout for 1024x600 screen without Camera Feed
				...
				if (!m_processedFrame.empty ()) {
					...
					cv::resize (m_processedFrame, resizedProcessed, cv::Size (440, 330));

					if (resizedProcessed.channels () == 1) {
						cv::cvtColor (resizedProcessed, resizedProcessed, cv::COLOR_GRAY2BGR);
					}

					resizedProcessed.copyTo (m_visualizationFrame (cameraRegion));

					cv::putText (m_visualizationFrame, "Lane Detection (Binary Mask)",
				...
				if (key == 'q' || key == 27) { // 'q' or ESC
					...
					if (recording_active) {
						recording_active = false;
						std::cout << "PARANDO gravação de trajetória ("
						          << recorded_waypoints.size () << " waypoints gravados)"
						          << std::endl;
					} else {
						std::cout << "Gravação já está PARADA" << std::endl;
					}
		...
		void drawCompactSystemStatus () {
			...
			cv::putText (m_visualizationFrame, "WP: " + std::to_string (recorded_waypoints.size ()),
			             cv::Point (status_x, status_y + line_height), cv::FONT_HERSHEY_SIMPLEX,
		...
		void createLaneVisualization (const cv::Mat &binary_mask) {
			try {
				...
				cv::findContours (binary_mask, contours, cv::RETR_EXTERNAL,
		...
		void drawTrajectoryVisualization (const cv::Rect &region) {
			try {
				...
				cv::putText (m_visualizationFrame, "MPC Trajectory",
				             cv::Point (region.x + 10, region.y + 25), cv::FONT_HERSHEY_SIMPLEX,
				...
				if (!m_predictedTrajectory.empty ()) {
					...
					for (size_t i = 1; i < m_predictedTrajectory.size (); i++) {
						cv::Point p1 = worldToScreen (m_predictedTrajectory[i - 1], region,
						                              center_x, center_y);
						cv::Point p2 =
		...
		void testZeroMQConnection () {
			...
			try {
				...
				if (m_laneDetectionSubscriber) {
					...
					zmq::pollitem_t items[] = {
					    {static_cast<void *> (m_laneDetectionSubscriber->getSocket ()), 0,
					     ZMQ_POLLIN, 0}};
