...
rsync -avz --update --exclude='.git' --exclude='build/' --exclude='*.o' --exclude='*.sh' --exclude='*.tmp' $LOCAL_PATH/ $JETSON_USER@$JETSON_HOST:$PROJECT_PATH/

echo "Compilando na Jetson Nano..."
ssh $JETSON_USER@$JETSON_HOST "cd $PROJECT_PATH && make \$1 -j4"
