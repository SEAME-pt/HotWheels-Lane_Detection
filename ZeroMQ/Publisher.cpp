...
Publisher::Publisher(int port)
    : context(1), publisher(context, ZMQ_PUB), joytstick_value(true),
      running(false) {
...
void Publisher::publish(const std::string &topic, const std::string &message) {
  std::cout << "[Publisher] Full message: " << topic << " " << message
            << std::endl;
...
void Publisher::setJoystickStatus(bool new_joytstick_value) {
  std::cout << "[Publisher] Publishing joystick_value: "
            << (joytstick_value ? "true" : "false") << std::endl;
...
void Publisher::publishInferenceFrame(const std::string &topic,
