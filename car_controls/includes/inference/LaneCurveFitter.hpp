...
public:
  ...
  LaneCurveFitter(float dbscanEps = 5.0f, size_t dbscanMinSamples = 20,
                  int numWindows = 20, int laneWidthPx = 300);

  std::vector<LaneCurve> fitLanes(const cv::Mat &binaryMask);
  std::optional<CenterlineResult>
  computeVirtualCenterline(const std::vector<LaneCurve> &lanes, int imgWidth,
                           int imgHeight);
...
private:
  ...
  slidingWindowCentroids(const std::vector<cv::Point> &cluster,
                         cv::Size imgSize, bool smooth);
  std::vector<float> fitCurve(const std::vector<float> &y,
