...
class LaneCurveFitter
{
  public:
    ...
    std::optional<CenterlineResult> computeVirtualCenterline(const std::vector<LaneCurve> &lanes,
  ...
  private:
    ...
    slidingWindowCentroids(const std::vector<cv::Point> &cluster, cv::Size imgSize, bool smooth);
