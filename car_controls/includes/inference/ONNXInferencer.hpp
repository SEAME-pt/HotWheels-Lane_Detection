...
class ONNXInferencer : public IInferencer {
	...
	public:
		 ...
		 */
		ONNXInferencer(const std::string &modelPath, const cv::Size &inputSize = cv::Size(640, 640),
		...
		cv::Mat postprocess(const std::vector<cv::Mat> &modelOutput, const cv::Mat &originalInput);
		...
		void setNormalizationParams(const cv::Scalar &mean, double scale, bool swapChannels) {
			meanValues = mean;
			scaleFactor = scale;
			swapRB = swapChannels;
		}
