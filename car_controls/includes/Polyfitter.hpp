...
public:
	...
	slidingWindowCentroids(const std::vector<double> &x,
						   const std::vector<double> &y, const cv::Size &imgShape,
						   bool smooth = false);
	...
	std::pair<Lane *, Lane *> selectRelevantLanes(std::vector<Lane> &lanes,
												  int imgWidth, int imgHeight);
	std::vector<double> linspace(double start, double end, int num);
	std::vector<double> interp(const std::vector<double> &xNew,
							   const std::vector<double> &x,
							   const std::vector<double> &y, double leftVal,
							   double rightVal);
	CenterlineResult computeVirtualCenterline(std::vector<Lane> &lanes,
											  int imgWidth, int imgHeight);
