...
/**
 * @brief Exemplo de como usar o InferenceManager com modelos PyTorch/ONNX e Keras
 */
void demonstrateInferenceManager() {
    ...
    if (manager.loadModel("lane_detection", onnx_model_path, ModelType::PYTORCH_ONNX)) {
    ...
    if (manager.loadModel("object_classifier", keras_model_path, ModelType::KERAS)) {
...
int main() {
    std::cout << "=== Demonstração do Sistema de Inferência Multi-Modelo ===" << std::endl;
