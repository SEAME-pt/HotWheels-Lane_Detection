...
void EngineController::set_steering (int angle) {
	...
	if (elapsed < 70) {
		...
		INFO_STREAM ("SERVO_PROTECTION") << "MPC timing conflict - comando " << elapsed
		                                 << "ms (min: 70ms) angle=" << angle << "° blocked";
	...
	if (std::abs (angle_diff) > max_change) {
		...
		INFO_STREAM ("SERVO_PROTECTION")
		    << "Rate limiting applied: " << angle_diff << "° -> " << (angle - last_angle)
	...
	if (std::abs (angle_diff) > 1) {
		INFO_STREAM ("SERVO") << "Ângulo: " << angle << "° (PWM: " << pwm << ")";
	}
