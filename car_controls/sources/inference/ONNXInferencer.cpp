...
cv::Mat ONNXInferencer::preprocess(const cv::Mat& input) {
    ...
    try {
        ...
        cv::dnn::blobFromImage(input, blob, scaleFactor, inputSize, meanValues, swapRB, false, CV_32F);
...
cv::Mat ONNXInferencer::postprocess(const std::vector<cv::Mat>& modelOutput, const cv::Mat& originalInput) {
    ...
    // Se a saída for uma máscara de segmentação, redimensionar para o tamanho original
