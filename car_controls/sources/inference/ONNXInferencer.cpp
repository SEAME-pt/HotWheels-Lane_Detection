...
ONNXInferencer::ONNXInferencer(const std::string &modelPath,
                               const cv::Size &inputSize,
                               const cv::Scalar &meanValues, double scaleFactor,
                               bool swapRB)
    : modelPath(modelPath), inputSize(inputSize), isModelLoaded(false),
      meanValues(meanValues), scaleFactor(scaleFactor), swapRB(swapRB) {

  if (!loadModel(modelPath)) {
    std::cerr << "[ONNXInferencer] Erro ao carregar modelo: " << modelPath
              << std::endl;
  }
}
...
bool ONNXInferencer::loadModel(const std::string &modelPath) {
  try {
    ...
    if (net.empty()) {
      std::cerr << "[ONNXInferencer] Erro: Modelo vazio ou inválido"
                << std::endl;
      return false;
    }
    ...
    isModelLoaded = true;
    std::cout << "[ONNXInferencer] Modelo carregado com sucesso: " << modelPath
              << std::endl;
    return true;

  } catch (const cv::Exception &e) {
    std::cerr << "[ONNXInferencer] Erro OpenCV ao carregar modelo: " << e.what()
              << std::endl;
    return false;
  } catch (const std::exception &e) {
    std::cerr << "[ONNXInferencer] Erro ao carregar modelo: " << e.what()
              << std::endl;
    return false;
  }
}
...
cv::Mat ONNXInferencer::preprocess(const cv::Mat &input) {
  ...
  try {
    ...
    cv::dnn::blobFromImage(input, blob, scaleFactor, inputSize, meanValues,
                           swapRB, false, CV_32F);

  } catch (const cv::Exception &e) {
    std::cerr << "[ONNXInferencer] Erro no pré-processamento: " << e.what()
              << std::endl;
    return cv::Mat();
  }
...
std::vector<cv::Mat> ONNXInferencer::infer(const cv::Mat &preprocessedInput) {
  ...
  try {
    ...
    std::cerr << "[ONNXInferencer] Erro durante inferência: " << e.what()
              << std::endl;
  } catch (const std::exception &e) {
    std::cerr << "[ONNXInferencer] Erro durante inferência: " << e.what()
              << std::endl;
...
cv::Mat ONNXInferencer::postprocess(const std::vector<cv::Mat> &modelOutput,
  ...
  if (modelOutput.empty() || originalInput.empty()) {
    std::cerr << "[ONNXInferencer] Saída do modelo ou imagem original vazia"
              << std::endl;
    return cv::Mat();
  }
...
cv::cuda::GpuMat
ONNXInferencer::makePrediction(const cv::cuda::GpuMat &gpuImage) {
...
void ONNXInferencer::doInference(const cv::Mat &frame) {
  ...
  std::cout << "[ONNXInferencer] Inferência concluída. Resultado: "
