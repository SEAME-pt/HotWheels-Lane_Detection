...
CameraStreamer::CameraStreamer(double scale)
  ...
  segmentationInferencer = std::make_shared<ONNXInferencer>(
      "/home/hotweels/dev/model_loader/models/model.onnx");
...
void CameraStreamer::detectionWorker() {
  while (m_running) {
    ...
    if (detectionBuffer.getFrame(frame)) {
      ...
      std::cout << "[CameraStreamer] Detecção YOLO temporariamente desabilitada"
                << std::endl;
    } else {
      std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
...
void CameraStreamer::captureLoop() {
  ...
  while (m_running) {
    // auto frame_start = std::chrono::high_resolution_clock::now();  // Para
