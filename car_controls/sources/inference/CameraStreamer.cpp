 ...
 * 			for autonomous vehicle perception. The CameraStreamer class orchestrates a multi-threaded
 * 			architecture that captures video frames from CSI cameras and processes them through
 * 			parallel AI inference pipelines for lane detection and object detection.
 * 			
 ...
 * 			                       ▼                                 ▼                                 ▼
 * 			               ┌──────────────────┐              ┌──────────────────┐           ┌──────────────────┐
 * 			               │ Segmentation     │              │ Detection        │           │ ZeroMQ           │
 * 			               │ Thread           │              │ Thread           │           │ Publisher        │
 * 			               │ (TensorRT)       │              │ (YOLO TensorRT)  │           │ (Results)        │
 * 			               └──────────────────┘              └──────────────────┘           └──────────────────┘
 * 			
 ...
 * 			- **Parallel Processing**: Independent threads for capture, lane detection, and object detection
 ...
 * 			5. **CUDA Resource Cleanup**: Unregisters CUDA graphics resources to prevent memory leaks
 ...
 * 			4. **Performance Optimization**: Minimal sleep when no frames available to prevent CPU spinning
 * 			
 * 			**Threading Behavior**:
 ...
 * 			3. **Result Processing**: Handles bounding boxes, confidence scores, and class predictions
 ...
 * 			1. **Frame Skipping**: Drops intermediate frames to reduce processing load and prevent buffer overflow
 ...
 * 			2. **Thread Signaling**: Sets m_running flag to false, signaling all worker threads to exit
