...
CameraStreamer::CameraStreamer(double scale)
  ...
  segmentationInferencer = std::make_shared<TensorRTInferencer>(
      "/home/jetson/models/lane-detection/model.engine");
  yoloInferencer = std::make_shared<YOLOv5TRT>(
      "/home/jetson/models/object-detection/yolov5m_updated.engine",
...
CameraStreamer::~CameraStreamer() {
  ...
  if (cuda_resource) {
    cudaGraphicsUnregisterResource(
        cuda_resource); // Unregister CUDA graphics resource
...
void CameraStreamer::segmentationWorker() {
  while (m_running) {
    ...
    if (segmentationBuffer.getFrame(frame)) {
      ...
      // std::cout << "[Segmentation] Inference time: " << duration_ms << " ms"
      // << std::endl;
    } else {
...
void CameraStreamer::captureLoop() {
  ...
  while (m_running) {
    ...
    if (elapsed >= 1) {
      std::cout << "Average FPS: " << frame_count / static_cast<double>(elapsed)
                << std::endl;
      start_time = now;
      frame_count = 0;
    }
