    ...
    try {
        switch (model_type) {
            ...
            case ModelType::KERAS:
                std::cout << "[InferenceManager] Carregando modelo Keras: " 
                          << model_name << std::endl;
        ...
        std::cerr << "[InferenceManager] Erro ao carregar modelo '" 
...
cv::Mat InferenceManager::predict(const std::string& model_name, const cv::Mat& image) {
    ...
    try {
        ...
        // Em uma implementação real, você modificaria IInferencer para ter um método predict
