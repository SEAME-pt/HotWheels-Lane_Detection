...
LaneCurveFitter::LaneCurveFitter(float eps, size_t minSamples, int windows,
                                 int laneWidthPx)
    : dbscanEps(eps), dbscanMinSamples(minSamples), numWindows(windows),
      laneWidthPx(laneWidthPx) {}

std::vector<cv::Point>
LaneCurveFitter::extractLanePoints(const cv::Mat &binaryMask) {
...
std::vector<int>
LaneCurveFitter::dbscanCluster(const std::vector<cv::Point> &points,
...
LaneCurveFitter::slidingWindowCentroids(const std::vector<cv::Point> &cluster,
                                        cv::Size imgSize, bool smooth) {
  ...
  for (int i = 0; i < numWindows; ++i) {
    ...
    if (!xAcc.empty()) {
      cx.push_back(std::accumulate(xAcc.begin(), xAcc.end(), 0.0f) /
                   xAcc.size());
      cy.push_back(std::accumulate(yAcc.begin(), yAcc.end(), 0.0f) /
                   yAcc.size());
    }
  }
...
bool LaneCurveFitter::isStraightLine(const std::vector<float> &y,
...
std::vector<LaneCurveFitter::LaneCurve>
LaneCurveFitter::fitLanes(const cv::Mat &binaryMask) {
  ...
  for (int label : uniqueLabels) {
    ...
    if (hasSignFlip(testCurve)) {
      std::tie(cy, cx) =
          slidingWindowCentroids(cluster, binaryMask.size(), true);
...
LaneCurveFitter::computeVirtualCenterline(const std::vector<LaneCurve> &lanes,
