...
LaneCurveFitter::clusterLanePoints (const std::vector<Point2D> &pts) {
...
LaneCurveFitter::slidingWindowCentroids (const std::vector<double> &x, const std::vector<double> &y,
...
std::vector<double> LaneCurveFitter::polyfit (const std::vector<double> &x,
...
std::vector<double> LaneCurveFitter::polyval (const std::vector<double> &coeffs,
...
std::vector<double> LaneCurveFitter::fitLaneCurve (const std::vector<double> &y,
...
void visualizeOverlay (const cv::Mat &maskGray, const cv::Mat &debugVis,
...
std::vector<LaneCurveFitter::LaneCurve> LaneCurveFitter::fitLanes (const cv::Mat &img) {
	...
	for (int label : uniqueLabels) {
		...
		for (size_t i = 0; i < points.size (); i++) {
			...
			if (enableDebugVis) cv::circle (debugVis, pt, 1, colors[label % colors.size ()], -1);
			if (labels[i] == label) {
		...
		double yStart =
		    std::max (0.0, *std::min_element (sortedCentY.begin (), sortedCentY.end ()) - 30);
		double yEnd = std::min ((double)img.rows,
		                        *std::max_element (sortedCentY.begin (), sortedCentY.end ()) + 10);
...
std::vector<double> LaneCurveFitter::interp (const std::vector<double> &xNew,
                                             const std::vector<double> &x,
                                             const std::vector<double> &y, double leftVal,
                                             double rightVal) {
...
LaneCurveFitter::CenterlineResult
LaneCurveFitter::computeVirtualCenterline (std::vector<LaneCurve> &lanes, int imgWidth,
                                           int imgHeight) {
