...
LanePostProcessor::LanePostProcessor(int minArea, int minLength, float angleThresh, float mergeDist)
    : minComponentSize(minArea), minComponentLength(minLength), angleThreshold(angleThresh),
...
LanePostProcessor::extractLaneInfo(const cv::Mat &mask, bool filterSize, bool filterLength) {
...
cv::Mat LanePostProcessor::renderFilteredMask(const std::vector<LaneInfo> &lanes,
                                              cv::Size shape) const {
	cv::Mat canvas(shape, CV_8UC3, cv::Scalar(0, 0, 0));
	for(const auto &lane : lanes) {
		cv::drawContours(canvas, std::vector<std::vector<cv::Point>>{lane.contour}, -1,
