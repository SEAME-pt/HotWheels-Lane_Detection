...
cv::cuda::GpuMat
LanePostProcessor::process(const cv::cuda::GpuMat &rawMaskGpu) {
  cv::cuda::GpuMat binaryMaskGpu;
  // Use CPU threshold since CUDA threshold is not available in this OpenCV
  // build
...
std::vector<LanePostProcessor::LaneInfo>
LanePostProcessor::extractLaneInfo(const cv::Mat &mask, bool filterSize,
                                   bool filterLength) {
  ...
  for (const auto &cnt : contours) {
    ...
    if ((!filterSize || area > minComponentSize) &&
        (!filterLength || length > minComponentLength) && cnt.size() >= 2) {
      ...
      if (vy != 0) {
        ...
        float x = x0 + vx * t;
        if (x >= 0 && x < width)
          intersections.emplace_back(x, 0);
        t = (height - 1 - y0) / vy;
        x = x0 + vx * t;
        if (x >= 0 && x < width)
          intersections.emplace_back(x, height - 1);
      }
      if (vx != 0) {
        float t = -x0 / vx;
        float y = y0 + vy * t;
        if (y >= 0 && y < height)
          intersections.emplace_back(0, y);
        t = (width - 1 - x0) / vx;
        y = y0 + vy * t;
        if (y >= 0 && y < height)
          intersections.emplace_back(width - 1, y);
      }

      if (intersections.size() >= 2) {
        lanes.push_back({static_cast<int>(lanes.size() + 1),
                         angle,
                         centroid,
...
bool LanePostProcessor::shouldMerge(const LaneInfo &a,
                                    const LaneInfo &b) const {
...
LanePostProcessor::getExtremities(const std::vector<cv::Point> &contour) const {
  double maxDist = 0;
...
void LanePostProcessor::drawLaneConnections(const std::vector<LaneInfo> &lanes,
                                            cv::Mat &canvas) const {
  for (size_t i = 0; i < lanes.size(); ++i) {
    for (size_t j = i + 1; j < lanes.size(); ++j) {
      if (shouldMerge(lanes[i], lanes[j])) {
        ...
        auto bestPair = *std::min_element(
            pairs.begin(), pairs.end(), [](const auto &p1, const auto &p2) {
              return cv::norm(p1.first - p1.second) <
                     cv::norm(p2.first - p2.second);
            });
...
cv::Mat
LanePostProcessor::renderFilteredMask(const std::vector<LaneInfo> &lanes,
                                      cv::Size shape) const {
  cv::Mat canvas(shape, CV_8UC3, cv::Scalar(0, 0, 0));
  for (const auto &lane : lanes) {
    cv::drawContours(canvas, std::vector<std::vector<cv::Point>>{lane.contour},
