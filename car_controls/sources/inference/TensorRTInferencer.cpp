...
{ // Initialize host output pointer to nullptr
    ...
    laneCurveFitter =
        new LaneCurveFitter(5.0F, 20, 20, 300); // Initialize lane curve fitter with parameters
...
cv::cuda::GpuMat TensorRTInferencer::makePrediction(const cv::cuda::GpuMat &gpuImage)
{
    ...
    /* 	cv::cuda::GpuMat postProcessedMaskGpu = lanePostProcessor->process(outputMaskGpu);
        ...
        auto centerlineOpt = laneCurveFitter->computeVirtualCenterline(lanes, maskCpu.cols,
       maskCpu.rows); if (!centerlineOpt.has_value()) { std::cout << "[DEBUG] No centerline could be
       computed." << std::endl;
        }
...
void TensorRTInferencer::doInference(const cv::Mat &frame)
{
    ...
    cv::cuda::resize(d_visualization, d_resized_mask, cv::Size(frame.cols * 0.5, frame.rows * 0.5),
