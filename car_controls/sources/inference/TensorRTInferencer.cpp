...
void TensorRTInferencer::Logger::log(Severity severity,
                                     const char *msg) noexcept {
  if (severity <=
      Severity::kWARNING)          // Only log warnings or more severe messages
    std::cout << msg << std::endl; // Print the message to the console
}

// Constructor: loads TensorRT engine, allocates memory and sets up execution
// context
TensorRTInferencer::TensorRTInferencer(const std::string &enginePath)
  ...
  engineData =
      readEngineFile(enginePath); // Load serialized engine file into memory

  runtime = nvinfer1::createInferRuntime(
      logger);    // Create TensorRT runtime with logger
  ...
  engine = runtime->deserializeCudaEngine(
      engineData.data(),
      engineData.size()); // Deserialize the engine from the loaded data
  if (!engine) {          // Check if deserialization failed
    throw std::runtime_error("Failed to deserialize engine");
  }

  context = engine->createExecutionContext(); // Create execution context from
                                              // the engine
  ...
  laneCurveFitter = new LaneCurveFitter(
      5.0f, 20, 20, 300); // Initialize lane curve fitter with parameters

  for (int i = 0; i < engine->getNbBindings();
       i++) {                        // Loop through all bindings
  ...
  inputDims = engine->getBindingDimensions(
      inputBindingIndex); // Get input tensor dimensions
  outputDims = engine->getBindingDimensions(
      outputBindingIndex); // Get output tensor dimensions

  if (inputDims.d[0] == -1) { // If input has dynamic batch dimension
    nvinfer1::Dims4 explicitDims(
        1, inputSize.height, inputSize.width,
        1); // Define explicit batch size and dimensions
    context->setBindingDimensions(
        inputBindingIndex, explicitDims); // Set explicit input dimensions
    inputDims = context->getBindingDimensions(
        inputBindingIndex); // Update inputDims after setting
  }

  outputDims = context->getBindingDimensions(
      outputBindingIndex); // Confirm and update outputDims

  for (int i = 0; i < outputDims.nbDims;
       i++) { // Check if any output dimension is dynamic
    if (outputDims.d[i] < 0) {
      throw std::runtime_error(
          "Output shape is undefined or dynamic"); // Throw error if output is
                                                   // not fully defined
    }
  }
  ...
  inputByteSize =
      inputElementCount * sizeof(float); // Calculate input buffer size in bytes

  outputElementCount = 1; // Initialize output element count
  for (int i = 0; i < outputDims.nbDims;
       i++) { // Multiply all output dimensions
    outputElementCount *= static_cast<size_t>(outputDims.d[i]);
  }
  outputByteSize = outputElementCount *
                   sizeof(float); // Calculate output buffer size in bytes

  cudaError_t status; // Define variable for checking CUDA errors

  status =
      cudaStreamCreate(&stream); // Create a CUDA stream for async operations
  if (status != cudaSuccess) {   // Check stream creation
    throw std::runtime_error("Failed to create CUDA stream: " +
                             std::string(cudaGetErrorString(status)));
  }

  status = cudaMalloc(&deviceInput,
                      inputByteSize); // Allocate device memory for input tensor
  ...
  status =
      cudaMalloc(&deviceOutput,
                 outputByteSize); // Allocate device memory for output tensor
  ...
  bindings.resize(
      engine->getNbBindings()); // Resize bindings array to number of bindings
...
std::vector<char>
TensorRTInferencer::readEngineFile(const std::string &enginePath) {
  std::ifstream file(enginePath,
                     std::ios::binary |
                         std::ios::ate); // Open file in binary mode, go to end
...
cv::cuda::GpuMat
TensorRTInferencer::preprocessImage(const cv::cuda::GpuMat &gpuImage) {
  ...
  if (gpuImage.type() != CV_8UC3 &&
      gpuImage.type() !=
          CV_8UC1) { // Check if input image is in expected format
    throw std::runtime_error(
        "Input image must be CV_8UC3 (color) or CV_8UC1 (grayscale)");
  }

  cv::cuda::GpuMat gpuGray;
  if (gpuImage.channels() > 1) { // If input has multiple channels (color)
    cv::cuda::cvtColor(gpuImage, gpuGray,
...
void TensorRTInferencer::runInference(const cv::cuda::GpuMat &gpuInput) {
  ...
  if (!context->enqueueV2(bindings.data(), stream,
                          nullptr)) { // Enqueue inference on the GPU
...
cv::cuda::GpuMat
TensorRTInferencer::makePrediction(const cv::cuda::GpuMat &gpuImage) {
  cv::cuda::GpuMat gpuInputFloat =
      preprocessImage(gpuImage); // Preprocess input image on GPU
  ...
  // Allocate or resize the output mask on GPU if it's not allocated or has
  // wrong size
  if (outputMaskGpu.empty() || outputMaskGpu.rows != height ||
      outputMaskGpu.cols != width) {
    outputMaskGpu = cv::cuda::GpuMat(height, width, CV_32F);
  }

  // Copy the raw prediction output from TensorRT device memory to
  // `outputMaskGpu`
  // - Assumes output is already in device memory (deviceOutput)
  // - No CPU-GPU transfer, all device-to-device
  cudaMemcpy2DAsync(outputMaskGpu.ptr<float>(), outputMaskGpu.step,
                    deviceOutput, width * sizeof(float), width * sizeof(float),
                    height, cudaMemcpyDeviceToDevice, stream);

  // post-process starts here
  /* 	cv::cuda::GpuMat postProcessedMaskGpu =
     lanePostProcessor->process(outputMaskGpu);
     ...
     laneCurveFitter->fitLanes(maskCpu); std::cout << "[DEBUG] Number of fitted
     lanes: " << lanes.size() << std::endl; auto centerlineOpt =
     laneCurveFitter->computeVirtualCenterline(lanes, maskCpu.cols,
     maskCpu.rows); if (!centerlineOpt.has_value()) { std::cout << "[DEBUG] No
     centerline could be computed." << std::endl;
          }
...
void TensorRTInferencer::initUndistortMaps() {
  ...
  cv::FileStorage fs("/home/hotweels/apps/camera_calibration.yml",
...
void TensorRTInferencer::doInference(const cv::Mat &frame) {
  ...
  cv::cuda::remap(d_frame, d_undistorted, d_mapx, d_mapy, cv::INTER_LINEAR, 0,
                  cv::Scalar(), cudaStream); // Undistort frame

  cv::cuda::GpuMat d_prediction_mask =
      makePrediction(d_undistorted); // Run model inference

  // Convert to 8-bit (0 or 255) in a new GpuMat
  cv::cuda::GpuMat d_mask_u8;
  d_prediction_mask.convertTo(d_mask_u8, CV_8U,
                              255.0); // Multiply 0/1 float to 0/255
  ...
  cv::cuda::resize(d_visualization, d_resized_mask,
                   cv::Size(frame.cols * 0.5, frame.rows * 0.5), 0, 0,
                   cv::INTER_LINEAR, cudaStream); // Resize for display
