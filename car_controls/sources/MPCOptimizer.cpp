...
MPCOptimizer::solve(double x0, double y0, double yaw0, double v0,
  ...
  nlopt::opt optimizer(nlopt::LD_SLSQP,
                       2 * MPCConfig::horizon); // SLSQP é melhor para MPC
...
double MPCOptimizer::_costFunction(const std::vector<double> &u,
  ...
  if (is_curve) {
    ...
    target_speed =
        std::max(_mpc.target_speed_curve_base,
  ...
  for (int t = 0; t < MPCConfig::horizon; ++t) {
    ...
    _kinematicModel(x, y, yaw, v, cte, epsi, throttle, steer,
                    _current_poly_coeffs);
...
void MPCOptimizer::_kinematicModel(
    double &x, double &y, double &yaw, double &v, double &cte, double &epsi,
  ...
  epsi = yaw - psides +
         (v / MPCConfig::wheelbase) * std::tan(steer) * MPCConfig::dt;
  yaw = _normalizeAngle(yaw);
  epsi = _normalizeAngle(epsi);
  v = std::max(0.0, std::min(v, 10.0));
}

double MPCOptimizer::_calculateCurveCurvature(
...
std::vector<double>
MPCOptimizer::_predictStateWithLatency(double x0, double y0, double yaw0,
