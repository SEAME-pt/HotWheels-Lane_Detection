...
MPCOptimizer::solve(double x0, double y0, double yaw0, double v0,
  ...
  // This function should implement the MPC algorithm to compute the optimal
  // control inputs based on the current state and reference trajectory.
...
double MPCOptimizer::_costFunction(const std::vector<double> &u,
  ...
  (void)lane_info; // Suppress unused parameter warning
  double cost = 0.0;
  ...
  for (int t = 0; t < MPCConfig::horizon; ++t) {
    ...
    if (t < (int)reference.size()) {
      ...
      double desired_heading =
          std::atan2(reference[t].y - y, reference[t].x - x);
