...
void YOLOv5TRT::loadEngine(const std::string &enginePath) {
  ...
  if (!engine) {
    std::cerr << "[ERRO] Falha ao desserializar o engine TensorRT!"
              << std::endl;
    exit(EXIT_FAILURE);
  }
...
std::vector<float> YOLOv5TRT::infer(const cv::Mat &image) {
  ...
  for (int c = 0; c < 3; c++) {
    memcpy(hostDataBuffer + c * 640 * 640, channels[c].ptr<float>(),
 ...
 */
std::vector<Detection> YOLOv5TRT::postprocess(const std::vector<float> &output,
                                              int num_classes,
                                              float conf_thresh,
                                              float nms_thresh) {
  ...
  for (size_t i = 0; i < dets.size(); ++i) {
    ...
    for (size_t j = i + 1; j < dets.size(); ++j) {
      ...
      // Calcular IoU
      float xx1 =
          std::max(dets[i].x - dets[i].w / 2, dets[j].x - dets[j].w / 2);
      float yy1 =
          std::max(dets[i].y - dets[i].h / 2, dets[j].y - dets[j].h / 2);
      float xx2 =
          std::min(dets[i].x + dets[i].w / 2, dets[j].x + dets[j].w / 2);
      float yy2 =
          std::min(dets[i].y + dets[i].h / 2, dets[j].y + dets[j].h / 2);
...
void YOLOv5TRT::process_image(const cv::Mat &frame) {
  ...
  std::vector<Detection> dets =
      postprocess(output, num_classes, conf_thresh, nms_thresh);

  for (const auto &det : dets) {
    // Converter coordenadas normalizadas para absolutas
    // Corrigir: tratar det.x, det.y, det.w, det.h como coordenadas absolutas
    // (input 640x640)
    ...
    if (x2 > x1 && y2 > y1) {
      ...
      std::cout << "Object found: " << className << " at (" << x1 << "," << y1
                << ")-(" << x2 << "," << y2 << ")" << std::endl;
      ...
      if (className != lastClassName ||
          elapsedMs > 2000) { // Only notify again if different or 2s passed
