...
std::vector<float> YOLOv5TRT::infer(const cv::Mat &image) {
	...
	for(int c = 0; c < 3; c++) {
		memcpy(hostDataBuffer + c * 640 * 640, channels[c].ptr<float>(), 640 * 640 * sizeof(float));
	}
	...
	cudaMemcpyAsync(outputHost, outputDevice, outputSize, cudaMemcpyDeviceToHost, stream);
 ...
 */
std::vector<Detection> YOLOv5TRT::postprocess(const std::vector<float> &output, int num_classes,
...
void YOLOv5TRT::process_image(const cv::Mat &frame) {
	...
	for(const auto &det : dets) {
		...
		if(x2 > x1 && y2 > y1) {
			...
			std::cout << "Object found: " << className << " at (" << x1 << "," << y1 << ")-(" << x2
			          << "," << y2 << ")" << std::endl;

			auto now = std::chrono::steady_clock::now();
			auto elapsedMs =
			    std::chrono::duration_cast<std::chrono::milliseconds>(now - lastNotificationTime)
			        .count();
