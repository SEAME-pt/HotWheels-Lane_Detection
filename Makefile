...
# Enhanced MPC main with direct PolyfitterInferencer integration (Simple version without Qt)
enhanced_mpc_main: enhanced_mpc_main.cpp \
	ZeroMQ/Publisher.cpp \
	ZeroMQ/Subscriber.cpp \
	car_controls/sources/Debugger.cpp \
	car_controls/sources/EngineController.cpp \
	...
	car_controls/sources/inference/CameraStreamer.cpp \
	car_controls/sources/MPCOptimizer.cpp
	@echo "=== Building Enhanced MPC with Direct PolyfitterInferencer Integration (Simple) ==="
	$(CXX) $(CXXFLAGS) $(QT5_CFLAGS) $(INCLUDE_PATHS) $^ -o $@ $(LDFLAGS) $(ALL_LIBS)
	@echo "✅ Enhanced MPC build complete! Run with ./enhanced_mpc_main"

# Clean
clean:
	rm -f $(OBJECTS) $(MOC_FILES) $(TARGET) enhanced_mpc_main lane_detection_video_test
